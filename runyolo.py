...
tiny_yolo_graph_file = "graph"                    #TODO
...
def filter_objects(inference_result, input_image_width, input_image_height):
    ...
    probability_threshold = range(0,1)           #TODO
    #probability_threshold: List[numpy.float16]  
...
def display_objects_in_gui(source_image, filtered_objects):
   ...
   # display_image = source_image.copy()             #uncommented by , made no difference
...
def main():
    ...
    #device, graph, fifo_in, fifo_out = do_initialize()      #add in by , not used
...
mvnc.Device(devices[0])
    ...
    while True:
        ...
        #cv2.imshow("display photographed image", input_image)
        #cv2.waitKey(1)
        #time.sleep(.25)
        ...
        graph.queue_inference_with_fifo_elem(fifo_in, fifo_out, input_image.astype(np.float32),
 None)
